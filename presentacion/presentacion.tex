...
\begin{frame}{Modelo de datos}
  \begin{columns}[c]
    \begin{column}{0.68\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.9\textheight,keepaspectratio]{modelo-datos.pdf}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{itemize}\small
        \item \textbf{15 clases} no dominio.
        \item \textbf{ID} como clave primaria.
        \item \texttt{RefreshToken} persistido → \textbf{revogable}.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
%============================== 7. DECISIÓNS TÉCNICAS =======================

% --- 6.0 Pila tecnolóxica: unha diapo por tecnoloxía. ---
% Cada unha usa unha copia editable do diagrama de «Arquitectura xeral»
% (assets/arquitectura-*.png) na que se pegan os logos manualmente.
% #1 imaxe do diagrama  #2 frase de xustificación (co nome en cursiva)
\newcommand{\pilaSlide}[2]{%
  \centering
  \includegraphics[width=0.84\textwidth]{assets/#1}\par
  \vspace{0.5em}
  \begin{minipage}{0.9\textwidth}\centering\small
    #2
  \end{minipage}}

\begin{frame}{Decisións técnicas · Pila tecnolóxica · Cliente (1/5)}
  \pilaSlide{arquitectura-cliente.png}{\textbf{React Native + Expo} permite desenvolver o cliente para iOS e Android cunha soa base de código en TypeScript, e Expo simplifica o build e o acceso ás APIs nativas do dispositivo.}
\end{frame}

\begin{frame}{Decisións técnicas · Pila tecnolóxica · Backend (2/5)}
  \pilaSlide{arquitectura-backend.png}{\textbf{Spring Boot} achega un ecosistema Java maduro con inxección de dependencias, seguridade e integración natural coa xeración de código a partir de OpenAPI.}
\end{frame}

\begin{frame}{Decisións técnicas · Pila tecnolóxica · Base de datos (3/5)}
  \pilaSlide{arquitectura-bd.png}{\textbf{PostgreSQL} é unha base relacional ACID, idónea para datos moi relacionados como viaxes, membros e gastos grazas á súa integridade referencial.}
\end{frame}

\begin{frame}{Decisións técnicas · Pila tecnolóxica · Almacenamento (4/5)}
  \pilaSlide{arquitectura-almacenamento.png}{\textbf{MinIO} é un almacén de obxectos compatible con S3 e autohospedable, que saca os documentos da base de datos e permite migrar a S3 sen tocar código.}
\end{frame}

\begin{frame}{Decisións técnicas · Pila tecnolóxica · Asistente IA (5/5)}
  \pilaSlide{arquitectura-ia.png}{\textbf{Ollama} aporta privacidade, evita custos de APIs externas e permite cambiar facilmente o modelo durante o desenvolvemento.}
\end{frame}

% --- 6.1 API-first / OpenAPI (logo oficial) ---
\begin{frame}{Decisións técnicas · API-first con OpenAPI (1/4)}
  \begin{columns}[c]
...
\begin{frame}{Decisións técnicas · Autenticación: JWT + refresh opaco (2/4)}
  \begin{columns}[c]
...
\begin{frame}{Decisións técnicas · Tempo real con SSE: chat e IA (3/4)}
  \begin{columns}[c]
...
\begin{frame}{Decisións técnicas · URLs prefirmadas (4/4)}
  \begin{columns}[c]
    \begin{column}{0.6\textwidth}
      \centering
      \resizebox{\linewidth}{!}{%
...
\begin{frame}{Demostración}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
      ...
      \vspace{0.8em}
      \begin{center}
        \href{run:assets/demo.mp4}{\chip{$\blacktriangleright$\; Reproducir vídeo da demostración}}
      \end{center}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
