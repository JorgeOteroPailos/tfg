...
\path{backend/compose.yaml}. O xeito máis sinxelo de poñelos en marcha, descargar o
...
\path{src/main/resources/application.yaml}. A documentación interactiva da API queda
dispoñible en \path{/swagger-ui.html}.
...
\path{frontend/constants/constants.tsx}, que debe apuntar ao enderezo accesible do
servidor dende o dispositivo. A aplicación pode abrirse en Expo Go ou nun emulador
(\texttt{npm run android} / \texttt{npm run ios}).

\section{Contrato OpenAPI e xeración de código}

A API especifícase en \path{backend/src/main/resources/openapi/}, cun documento
...
\path{backend/build/reports/jacoco/test/html/index.html}. A análise estática SpotBugs
...
\path{application.yaml}. Para un despregamento en produción recoméndase externalizar
