...
Como se mencionou anteriormente, Telaria non traballa con roles: o sistema 
recoñece un único tipo de actor, o usuario rexistrado. Todos os membros dunha 
viaxe teñen as mesmas capacidades e acceden en igualdade de condicións a todas 
as funcionalidades, sen que ningún dependa doutro, o que reforza o modelo 
colaborativo.
...
As interfaces de software coas que interactúa o sistema son as seguintes:
...
Os requisitos funcionais da aplicación amósanse no Cadro~\ref{tab:requisitos-funcionais},
agrupados en sete áreas: autenticación e conta (RF01-RF05), viaxes e membros (RF06-RF09),
gastos e liquidacións (RF10-RF11), eventos (RF12), documentos (RF13), chat e asistente
de IA (RF14-RF15) e amizades (RF16-RF17). A prioridade asignada a cada requisito
(alta, media ou baixa) reflicte a súa relevancia para a funcionalidade nuclear do produto.

\begin{longtable}{@{}p{1.2cm}>{\raggedright\arraybackslash}p{3cm}p{8.5cm}p{1.5cm}@{}}
  \caption{Requisitos funcionais da aplicación.}
  \label{tab:requisitos-funcionais} \\
  \toprule
  \textbf{ID} & \textbf{Nome} & \textbf{Descrición} & \textbf{Prior.} \\
  \midrule
  \endfirsthead
  \caption*{Cadro~\ref{tab:requisitos-funcionais} (continuación)} \\
  \toprule
  \textbf{ID} & \textbf{Nome} & \textbf{Descrición} & \textbf{Prior.} \\
  \midrule
  \endhead
...
Os requisitos non funcionais definen as calidades que debe presentar o sistema con
independencia das funcionalidades que ofrece: non describen \textit{que} fai a
aplicación, senón \textit{como} debe comportarse. Agrúpanse en sete categorías:
seguridade, privacidade e protección de datos, rendemento, usabilidade, mantibilidade,
portabilidade e despregamento, interoperabilidade e fiabilidade. Dado o carácter
colaborativo de Telaria —onde varios usuarios comparten datos sensibles dentro dunha
viaxe—, os requisitos de seguridade e privacidade teñen un peso especial no deseño
do sistema.
...
\begin{itemize}
  \item \textbf{RNF-RE-01}. A entrega de mensaxes do chat e das respostas do asistente de IA realízase en tempo real mediante eventos enviados polo servidor (SSE).
  \item \textbf{RNF-RE-02}. A xeración de miniaturas dos documentos de imaxe execútase de forma asíncrona, fóra da ruta de petición, para non penalizar a latencia da subida nin da listaxe.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{RNF-FI-02}. O sistema debe garantir a consistencia entre os rexistros de documentos da base de datos e os ficheiros presentes no almacén de obxectos, evitando a acumulación de datos inconsistentes.
\end{itemize}
...
Amazon~S3 (onde o seu uso se factura por consumo), MinIO execútase na propia
infraestrutura do proxecto, despregándose como un contedor (Docker / Podman) xunto co
resto de servizos. Isto
